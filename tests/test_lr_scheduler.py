...
def test_cosine_scheduler_monotonic_after_warmup():
    sched = LRScheduler(
        "cosine", learning_rate=1e-3, min_lr=1e-5, warmup_iters=10, lr_decay_iters=100
    )
    # Warmup increases LR.
...
def test_state_dict_roundtrip():
    ...
    sched2 = LRScheduler(
        "linear", learning_rate=0.0, min_lr=0.0, warmup_iters=0, lr_decay_iters=1
    )
...
def test_constant_scheduler():
    sched = LRScheduler(
        "constant", learning_rate=1e-3, min_lr=1e-5, warmup_iters=0, lr_decay_iters=100
    )

...
from data.openwebtext import MemmapDataset
...
def main():
    ...
    model, ckpt = load_model_from_checkpoint(args.checkpoint, device=device, model_type=args.model)
    ...
    inputs, targets = build_dummy_data(args.batch_size, args.block_size, args.steps, device)
    ...
    with prof:
        for step, (x, y) in enumerate(loader):
            if step >= args.steps:
                break
            with torch.cuda.amp.autocast(device_type="cuda", dtype=torch.bfloat16 if torch.cuda.is_available() and torch.cuda.is_bf16_supported() else torch.float16):

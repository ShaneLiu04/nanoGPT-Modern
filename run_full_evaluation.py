...
def load_model(path, model_type, device):
    ...
    config = ckpt.get("config", ModernGPTConfig() if model_type == "modern" else BaselineGPTConfig())
...
def evaluate_model(model, data, tokenizer, device, max_response_len=20):
    ...
    with torch.no_grad():
        for prompt in prompts:
            ...
            if hasattr(model, "generate") and "use_cache" in model.generate.__code__.co_varnames:
                generated = model.generate(input_ids, max_new_tokens=max_response_len, temperature=1.0, top_k=50, use_cache=True)
            else:
                generated = model.generate(input_ids, max_new_tokens=max_response_len, temperature=1.0, top_k=50)
...
def main():
    ...
    for name, path, mtype in checkpoints:
        ...
        for level, data in eval_data.items():
            ...
            print(f"  {level:8s}: acc={res['accuracy']:.3f}, reward={res['reward']:.3f}, fmt={res['format_pass_rate']:.3f}, proc={res['process_score']:.3f}, invalid={res['invalid_rate']:.3f}")

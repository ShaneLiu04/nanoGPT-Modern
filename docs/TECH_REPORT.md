...
| TD-02 | MoE 实现仅具示意性 | P2 | 模型性能 | 未实现 grouped GEMM | 当前为 Python for-loop 路由，存在 CPU-GPU 同步 |
...
| TD-05 | 缺乏容器化与 CI/CD | P2 | 工程成熟度 | 项目阶段 | 建议 Q3 引入 |
| TD-06 | 缺少分布式训练自动化测试 | P2 | 可靠性 | 单卡环境限制 | 建议引入多 GPU CI runner |
| TD-07 | 类型注解覆盖不完整 | P3 | 可维护性 | 历史遗留 | 核心模块已通过 mypy，持续扩展中 |
| TD-08 | 投机解码仅支持 batch_size=1 | P3 | 推理场景 | 实现复杂度 | 当前为研究特性，生产需扩展 |
| TD-09 | 缺少生产级推理服务框架 | P3 | 部署能力 | 项目定位 | 未来引入 vLLM / TGI |
| TD-10 | 数据管道 shuffle 非全局 | P3 | 数据质量 | 内存限制 | 使用 chunk-level shuffle_buffer |
...
| **P1 - 短期** | 影响效率或稳定性，应在 1-2 个月内修复 | TD-03, TD-04 |
| **P2 - 中期** | 影响工程成熟度，应在 3-6 个月内完成 | TD-02, TD-05, TD-06 |
| **P3 - 长期** | 优化项，不影响当前功能 | TD-07, TD-08, TD-09, TD-10 |
...
| 任务 | 目标 | 交付物 | 负责人 |
|------|------|--------|--------|
| 50M 模型完整训练验证 | 在真实 OpenWebText 上训练 18k iters，验证 README 目标指标 | 训练报告 + 检查点 | 核心研发 |
| 容器化部署 | 提供 Dockerfile + docker-compose，支持一键启动训练 | Dockerfile + 文档 | 运维 |
| CI/CD 接入 | GitHub Actions 自动运行 pytest + mypy + lint | `.github/workflows/ci.yml` | 核心研发 |
| 日志监控完善 | 引入 Prometheus + Grafana 监控 GPU 利用率与训练指标 | 监控面板配置 | 运维 |

#### Q4 2026 (10-12月)

| 任务 | 目标 | 交付物 | 负责人 |
|------|------|--------|--------|
| 多卡分布式训练验证 | 在 2-4x GPU 上验证 DDP/FSDP 正确性与性能 | 分布式训练报告 | 核心研发 |

    ...
    for model in ["baseline", "modern"]:
        for use_cache in [False, True]:
            ...
            if model == "baseline" and use_cache:
                continue
            cmd = [
                sys.executable, "inference/generate.py",
                ...
                "--output_json", f"out/ablation_{model}_cache{use_cache}.json",
            ]
            stdout = run(cmd)
...
def train_ablations(args):
    ...
    for model in ["baseline", "modern"]:
        ...
        cmd = [
            sys.executable, "training/train_pretrain.py",
            ...
            "--eval_interval", str(max(1, args.max_iters // 5)),
            "--eval_iters", "10",
            "--log_interval", "50",
            "--device", args.device,
            "--num_workers", "0",
        ...
        for line in stdout.splitlines():
            if "val loss" in line.lower():
                try:
                    final_val = float(line.split("val loss")[-1].strip().rstrip(",").split()[0])
                except Exception:
                    pass
        results.append({
            "model": model,
...
def main():
    ...
    if args.mode == "inference":
        ...
        print("|-------|-----------|------------|-----------|----------|-------|")
        for r in results:
            print(f"| {r.get('model')} | {r.get('use_cache')} | "

...
from data.arithmetic import generate_easy, generate_medium, generate_hard, ArithmeticDataset, collate_fn
...
def get_args():
    ...
    parser.add_argument("--init_from", type=str, required=True, help="Path to pretrain checkpoint")
    ...
    parser.add_argument("--variant", type=str, default="sft-only", choices=["sft-only", "sft-continued"])
    parser.add_argument("--num_train", type=int, default=5000, help="Samples per difficulty level for training")
    parser.add_argument("--num_val", type=int, default=500, help="Samples per difficulty level for validation")
...
class SFTTrainer(BaseTrainer):
    ...
    def _build_data(self):
        ...
        train_ds = ArithmeticDataset(train_data, self.tokenizer, max_length=args.max_length, pre_tokenize=True)
        val_ds = ArithmeticDataset(val_data, self.tokenizer, max_length=args.max_length, pre_tokenize=True)
    ...
    def _build_model(self):
        ...
        if args.resume:
            ...
            self.model, _ = load_model_from_checkpoint(args.init_from, device=self.device)
            ...
            for batch in self.train_loader:
                ...
                if self.global_step % args.eval_interval == 0 and self.master_process:
                    ...
                    if avg_val < self.best_val_loss:
                        ...
                        self.save_checkpoint(f"best_{args.variant}.pt", self.global_step, self.best_val_loss)

                self.global_step += 1

        if self.master_process:
            self.save_checkpoint(f"final_{args.variant}.pt", self.global_step, self.best_val_loss)
